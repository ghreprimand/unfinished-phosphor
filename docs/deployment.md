...
The intended public address is `https://phosphor.unfinished-works.com/`. It has not been activated.
...
After activation, add the public URL to the README and GitHub repository's About/Website field. Set the playground's canonical URL and `og:url` to that address. Capture a representative demo image for social previews after the public palette selection is final; use synthetic demo content.

## What the build contains

`npm run build` creates the static pages and assets, then adds `LICENSE`, `THIRD_PARTY_NOTICES.md`, and `source.tgz` to `dist/`. Both pages link to these files. The archive contains an explicit list of standalone source files, build configuration, lockfile, notices, and tests; it does not archive the entire working directory or Git history. Update that list in `scripts/package-demo.mjs` when adding required source files.
...
Before announcing the public site, check both pages, keyboard controls, narrow screens, and the license/source downloads at the actual deployed address. Put the demo URL first and the repository URL second in a post. Describe it as an experimental CRT CSS/JavaScript toolkit, credit the adapted palettes, and avoid implying an npm package exists.
